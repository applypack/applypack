...
| Matches found outside the alert window (held, then sent as one message) | `Job.alertHeldAt` set in `jobs/process-jobs.ts`; delivery `jobs/alert-delivery.ts:deliverHeldAlerts` (called at the top of the fetch tick, above the pause) over the pure `jobs/held-alerts.ts:groupHeldByTarget`; count `countHeldAlerts` on `/` and `/settings` |
