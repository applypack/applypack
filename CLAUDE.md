...
| The five sentences under the score, and why "send it" waits | `src/web/score-lines.ts` (pure, docs/score-lines-plan.md) — three lines that made the number (Requirements, Core stack, First glance) and two it does not count (Shown at work, To confirm), rendered by `ScoreBreakdownChips`. `readyToApply` gates "stop polishing, send it" on the open edits, the gates and any must-have named only on a skills line, not on the number alone |
...
| Bulk intake: zip, dedupe, the caps | `src/screening/intake.ts` (pure) over `resume/zip.ts:readZipEntries`; duplicates by text hash, email, or SimHash within `MAX_HAMMING_DISTANCE` |
| The batch: one call per applicant, resumable | `src/screening/batch.ts:startScreeningRun` — `listPending` is the queue, every verdict is written as it arrives, the in-memory state is only the counter; `POST /screen/:id/run`, polled by `public/screen.mjs` |
...
| Add applicants | `/screen/:id` → Applicants → files or a .zip → Add applicants. Unreadable files and copies of the same person stay in the list unscored |
| Score them, see the order | `/screen/:id` → Score N applicants (one call each, `AI_CONCURRENCY` at a time; the page updates itself, a restart resumes from what is missing). Buckets: Priority to talk to / Ask first / Did not pass a gate; then score, then confidence. `*` after a score = capped, the scorecard says why |
