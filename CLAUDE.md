...
| Get the DOU / Djinni / Arbeitnow feeds a search's countries call for | `/companies` → **Add sources** → "Sources for your searches" — the button carries how many are waiting (shown when a running search names UA, DE, AT, CH or GB; Add (off) probes first, then the row's toggle enables it) |
...
| See only roles you could actually take from where you live | `/jobs` → **Filters** → Show → "Open to me" (reads each search's own location verdict; set "I live in" on `/settings` → Profile → Location first) |
...
| Compare a resume with a posting | `/jobs/:id` → **Resume match** tab — **Compare** = quick check (keywords, gates, score), **Full analysis** = also the edit suggestions (ADR 0029) |
...
| Compare a found job with a file that is not in Resumes, or with pasted text | `/jobs/:id` → **Resume match** tab → **Compare a file or pasted text →** (opens `/target?job=:id` with the job picked), or menu → Compare → "One of your jobs". Nothing is added to Resumes |
