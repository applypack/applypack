...
| The batch: one call per applicant, resumable, draining | `src/screening/batch.ts:startScreeningRun` — `listPending` is the queue, every verdict is written as it arrives, the in-memory state is only the counter, and the run asks the queue again when its batch is done (an upload starts it; a second upload lengthens it); `{ again: ids }` is the bulk "Score again"; `POST /screen/:id/run`, polled by `public/screen.mjs` |
