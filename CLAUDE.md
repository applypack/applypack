...
| Why "matched" always agrees with the missing-keyword chips | `src/resume/keyword-anchor.ts:anchorStatuses` (pure) — a `present` the browser matcher cannot find in the resume text becomes `add`, so the table, the chips and the live estimate cannot contradict each other (4% of present keywords on the live corpus) |
...
| Why a red flag about a primary keyword sometimes costs nothing | `primaryWritten` in `src/resume/score.ts` — the cap asks what the candidate HAS (present or add), the red-flag exemption asks what the resume SPELLS (present only). A flag restating a primary the text never writes is already paid for by half the keyword credit |
