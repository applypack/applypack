...
| Apply a suggestion, or your own version of it | `/jobs/:id/target` → the card's **Apply**, or **Edit & apply** to change the wording first. **Remove** on a removal, **Skip** to set one aside, **Undo** on anything done. Nothing is saved until you Save as vN — the edits live in the editor |
