...
| The screening prompt, one answer per criterion, the reply shape | `src/screening/prompts.ts:buildScreenPrompt` / `ScreenReplySchema` (v2): `answerShape(criterion)` says whether a criterion is answered as a rung (`EVIDENCE_RUNGS`: absent · listed · project · role · production), a status (pass / partial / unknown / fail), a level, an impact grade, the overall read, or read off the roles; in the fence registry |
