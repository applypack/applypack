...
| Where that shape comes from, and what stops the model rewriting the resume into it | its own call, `src/resume/structure.ts:structureResume` over `prompts.ts:buildStructurePrompt` (since v1.66.0 — the scan no longer carries it; `ScanSchema.structure` stays optional so an old reply parses), started lazily by `/resumes/:id/render` on the first visit and by its "Read the shape with AI" button → `src/resume/structure-anchor.ts:anchorStructure` (pure: every string must be a verbatim span of `resume.text`; the drop count is the regression metric, logged on every reading) → `store.ts:saveResumeStructure`; the deterministic floor when the column is NULL is `src/resume/structure-from-text.ts` |
...
| Posting (one of your jobs, or pasted) + resume → one-shot targeted analysis | `/target` — `src/web/routes/target.tsx`: a stored job goes straight to `comparison-run.ts:startComparison`; a pasted one composes `jobs/manual-job.ts` + `resume/match.ts`. The picker is `pages/job-picker.tsx` over `job-pick.ts:listPickableJobs` (shared with `/letter`; `/screen/new` uses the component with its own list), `?job=` preselects. The resume half is `resume-source.ts:resolveResumeSource` (shared with `/letter`): upload/paste land on the hidden scratch resume, whose comparisons keep their text snapshot and whose letters retire when the text changes (`store.ts:upsertScratchResume`) |
...
| Compare-run progress pages (async classify/scan/match) | `src/web/target-runs.ts` (in-memory registry) + `src/web/pages/target-run.tsx`; started by `/target`, and by `src/web/comparison-run.ts:startComparison` — the one path `/jobs/:id/match`, `/jobs/:id/target/reupload` and `/target`'s "One of your jobs" all go through, so Compare, "Analyse my resume again", a fresh file and a file against a stored job behave identically |
| Which resume a job page preselects | `src/resume/pick.ts:preselectResume` — the active profile's `resumeId` first, then `pickResumeForJob` (skill-tag overlap); the option's wording (file kind, version, why) is `src/web/resume-label.ts` |
...
| The gold set: a ranked folder through the screening's own path, τ / precision@5 / stability / leaks / gate confusion | `npm run bench:screen [-- --set <name> --runs 2 --out f.json]` → `src/scripts/screen-bench-once.ts` over the pure `src/screening/bench.ts` (`kendallTau`, `precisionAtK`, `stability`, `gateConfusion`, `parseRanking`); sets live in `src/screening/fixtures/gold/<set>/` (README there); spends AI, writes nothing, hand-run |
...
| Compare a found job with a file that is not in Resumes, or with pasted text | `/jobs/:id` → Resume match → **Compare a file or pasted text →** (opens `/target?job=:id` with the job picked), or menu → Compare → "One of your jobs". Nothing is added to Resumes |
| Check whether a posting is real | `/jobs/:id` → "Is this job real?" → Verify (web search, 2-4 min) |
| Replace a truncated or pasted description with the company's own listing | `/jobs/:id` → "Is this job real?" → after a deep check that found the company's page: **Refresh the description from it** → a line-by-line preview → **Replace the description and re-classify**; the original stays (**Restore the original** above the description), the next comparison rebuilds its keywords (ADR 0043) |
| Draft / edit / copy a cover letter | `/jobs/:id` → "Cover letter" card (Generate / Regenerate; edits autosave and re-check facts); **Addressed to** greets a person by name — prefilled from the verification's `named_humans` finding (`resume/addressee.ts`, pure), editable, the finding shown beside it |
...
| Edit what the screen checks | `/screen/:id` → "What this screen checks": one row per criterion — kind, the words ("Playwright / Cypress !", "0–2 years", "fintech: 3+", "has led a team of three or more"), Gate / Scored / Note, the stars, Remove — and the last row adds your own (pick the kind, type it, choose yes/no or how-much). "Start over from a shape of hiring" re-reads the posting as Standard / Junior / Senior / Regulated / Agency. Save bumps the rubric version; a row naming age, gender, family, origin or health is refused |
