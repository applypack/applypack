...
| `career-ops` | third-party clone (`santifer/career-ops`, MIT) — `discover-ats.mjs`, `detect-reposts.mjs`, `check-liveness.mjs`, `dedup-tracker.mjs`, `tracker`, `followup`, `interview-prep` modes | Overlaps heavily with this project (it is the same idea, file-based). Do **not** vendor it; mine three pieces: repost detection (same title+company re-posted within N days), liveness check (posting URL 404 → auto-archive), follow-up cadence for `applications`. |
