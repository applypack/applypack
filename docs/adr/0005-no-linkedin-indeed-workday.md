...
| GermanTechJobs.de / DevITjobs.uk / DevITjobs.nl | adopted (v1.35.0) | `/rss` is the sites' own syndication (`utm_source=our_rss_feed` in every link); `robots.txt` disallows only apply links, push-worker files and `*.gz` / `*.php`; ETag + Last-Modified honoured (304), one conditional request per site per tick (2026-09-03) |
