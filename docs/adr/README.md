...
An accepted ADR is never rewritten. When a later ADR changes it, the newer
one says so in its status line and the older one links back ("amended by",
"extended by", "superseded by"). A fact that changed without a new decision
gets a dated addendum at the end of the ADR.
...
- [0019 — Source health is a per-company streak; `empty` resets it but does not prove health](./0019-source-health-streaks.md) — *amended by 0035*
- [0020 — The fact gate blocks fabrication, not imprecision](./0020-fact-gate-blocks-fabrication-not-imprecision.md)
- [0021 — Cover letters generate from stored inputs only](./0021-cover-letters-stored-inputs-only.md) — *extended by 0042*
