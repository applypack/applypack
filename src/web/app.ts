/*
 * The dashboard assembled — middleware, static files, every route, the error
 * handler — and not served: server.ts listens, scripts/route-smoke.ts calls
 * app.request() against it in-process, which is how every route gets an
 * automated request without a browser (TASKS §20 route-smoke-ci).
 */
