...
export const JobNewPage: FC<{ flash?: FlashMessage | null }> = ({ flash }) => (
  <Layout title="Paste a job" active="jobs">
    ...
    <PageHeader title="Paste a job">
      <p class="mt-1 text-sm text-ink-faint">
        For postings the fetchers don't see (LinkedIn, a recruiter's email, a friend's
        referral). It becomes a normal job: scored against your profile, then Verify
        checks the company is real and Compare tells you what to change in the resume.
      </p>
    </PageHeader>
    <Flash flash={flash} />
    ...
    <Card>
      <form method="post" action="/jobs/new" class="grid gap-4 sm:grid-cols-2">
        <Field label="Company" class="sm:col-span-1">
          <Input type="text" name="companyName" required maxlength="200" placeholder="Acme Corp" />
        </Field>
        <Field label="Job title" class="sm:col-span-1">
          <Input type="text" name="title" required maxlength="200" placeholder="Senior PHP Developer" />
        </Field>
        <Field label="Posting URL" hint="Optional — helps verification find the original." class="sm:col-span-1">
          <Input type="url" name="url" placeholder="https://…" />
        </Field>
        <Field label="Location" hint='As written in the posting: "Remote (US)", "Austin, TX (hybrid)".' class="sm:col-span-1">
        ...
        <Field
          ...
          class="sm:col-span-2"
        >
          <Textarea name="description" rows={18} required minlength="200" placeholder="About the role…" />
        </Field>
        <div class="flex items-center gap-3 sm:col-span-2">
          <Button size="lg">Save job</Button>
          <Hint>Runs the classifier once (a few seconds). Verify and Compare are separate buttons on the job page.</Hint>
        </div>
      </form>
    </Card>
  </Layout>
