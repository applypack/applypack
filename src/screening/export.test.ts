...
test('toCsv quotes what needs quoting and marks the gates', () => {
  ...
  assert.ok(lines[0]!.startsWith('﻿Applicant,Name,File,Status,Bucket,Score,Confidence,Gate: EU work authorisation,Gate: German B2,'));
  ...
  assert.ok(lines[2]!.endsWith('no text layer'));
});
...
test('toMarkdown groups by bucket and lists the unread files', () => {
  ...
  assert.match(md, /\| №2 \| Олена, "QA" \| 81 \| high \| ✓ \| \? \| 6\/7 \| 9 \| senior \| To interview \|/);
